...
** System packages
   ...
   - [[https://prettier.io/][prettier]] :: Used to prettify HTML & JS code upon save. See: [[#development][development]].
     - [[https://github.com/codesuki/add-node-modules-path][add-node-modules-path]] :: Utilized local =.prettierrc=, provided that =prettier= package is also listed on =package.json=.
