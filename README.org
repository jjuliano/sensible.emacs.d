...
** Files
  ...
  Files are structured in the following tree.
   ...
   #+BEGIN_SRC text
     ...
     ├── config
     │   └── 00-packages.el        // default package configurations
     |   └── 01-editor.el          // editor configurations
     ├── config.el                 // loads all the *.el files in the config/ and personal/ directory
     ├── init.el                   // contains the org-directory and about-me settings
     ├── personal/.gitkeep.        // added to .gitignore, to store all custom user configurations
...
** System packages
   ...
   - [[https://cmake.org/][cmake]] :: Used by [[#term][terminal emulator]].
     - shell-side configs needed to be added to .zshrc or .bashrc see: [[https://github.com/akermu/emacs-libvterm#shell-side-configuration][shell-side configuration]]
